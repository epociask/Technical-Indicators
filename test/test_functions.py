...
class TestFunctions(unittest.TestCase):
    ...
    def testSMA(self): 
        expected = 40.75
        actual = indicators.SMA(values, 4)
        self.assertEqual(expected, actual)

    def testMOMENTUM(self):
        expected = 39
        actual = indicators.MOMENTUM(values, 3)
        self.assertEqual(expected, actual)

    def testSDV(self): 
        expected = 22.23 
        actual = indicators.SDV(values, 4)
        self.assertAlmostEqual(expected, float_format(actual))

    def testBB(self): 
        a1, a2, a3 = indicators.BOLINGER_BANDS(values, 4, 2)
    ...
    def testFIBONACCI(self): 
        ...
        print("values ", values)
        a1, a2, a3 = indicators.FIB(values, values.shape[0])
        self.assertListEqual(expected, [a1, a2, a3])

    def testWMA(self):
        expected = 89.2
        actual = indicators.WMA(np.array([90.0, 89.0, 88.0, 89.0]), 4, 10)
        self.assertEqual(expected, float_format(actual))

    def testDERIVATIVE(self):
        expected = 1
        actual = indicators.DERIVATIVE(values1, 2)
        print(actual)
    ...
    def testEMA(self):
        ...
        actual = np.array(indicators.EMA(vals, 7))
        print("actual", actual) 
